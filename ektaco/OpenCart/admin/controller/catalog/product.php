...
fclose($output); // Closing the File
	...
	//	'href' => $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . $url, 'SSL'), 'separator' => ' :: ');
		...
		$data['products'] = array();
        	...
        	/*tutorial*/
		$data['export_csv'] = $this->url->link('catalog/product/exportCSV', 'token=' . $this->session->data['token'] . $url, true);
	        /*CompuCash*/
		$data['import_cc_products'] = $this->url->link('catalog/product/importCCProducts', 'token=' . $this->session->data['token'] . $url, true);

		$data['cancel'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . $url, true);
		// cancel url
	    	$data['action'] = $this->url->link('catalog/product/importCSV', 'token=' . $this->session->data['token'] . $url, true);
		// current page url
	...
	/*Compucash IdentityServer token*/
	...
	/* Compucash get products from API */
	protected function getProducts($idtoken)
	{
	    ...
	    /* Compucash set Product.externalId */
       ...
       $this->load->model('catalog/product');
		...
		foreach ($languages as $language) {
		   $language['language_id'];
                   //todo: kõikides keeltes API päring
		}*/
		...
		if (count($products) > 0)
		{
			foreach ($products as $product) {
				...
				$productCategory = (!empty($product->ExternalGid)) ? $product->ExternalGid : 0;
	   	...
	   	$this->session->data['success'] = 'Products Successfully Imported!'; //success message
...
/* Compucash OpenCart productId mapping */
